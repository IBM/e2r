...
class FB15KReasonData:
  ...
  def __init__(self, path, negSampleSizeRatio, mapLoadPath=None, mapLoadNamePrefix=None):
    ...
    if mapLoadPath==None and mapLoadNamePrefix==None:
      self.e2id, self.uc2id, self.bc2id = self.getEntitiesConceptsMap(path+'/freebase_mtr100_mte100-train.txt')
    else:
    ...
    self.aboxUTripleLst, self.aboxBTripleLst = self.readAboxTripleList(path+'/freebase_mtr100_mte100-train.txt')
    ...
    self.allAboxUTripleLst, self.allAboxBTripleLst = self.readAllAboxTripleList(path+'/freebase_mtr100_mte100-train.txt', path+'/freebase_mtr100_mte100-valid.txt', path+'/freebase_mtr100_mte100-test.txt')
    self.allUCMemberMap, self.allBCMemberMap = self.getAllClassMemberMap()
    self.testAboxUTripleLst, self.testAboxBTripleLst = self.readAboxTripleList(path+'//freebase_mtr100_mte100-test.txt')
